...
Usage (from repo root):
    python3 scripts/package_tools.py vcpkg 2026-02-21
    python3 scripts/package_tools.py vcpkg 2026-02-21 --output-dir ./packages
    python3 scripts/package_tools.py clang-tools 22.1.2     # future

Outputs tarballs + checksums ready to upload as GitHub Release assets.
"""
...
def package_vcpkg(version: str, output_dir: Path):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for platform_arch, binary_path in binaries.items():
            ...
            # Compatibility symlink: upstream vcpkg.cmake looks for $VCPKG_ROOT/vcpkg
            compat_link = vcpkg_dir / "vcpkg"
            if compat_link.exists() or compat_link.is_symlink():
                compat_link.unlink()
            os.symlink("bin/vcpkg", compat_link)
            print(f"    ✓ vcpkg -> bin/vcpkg compat symlink")
    ...
    print(f"  Then update tools/vcpkg.json with the URLs and checksums above.")
